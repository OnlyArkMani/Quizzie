...
interface ExamState {
  ...
  // Actions
  initExam: (examId: string, questions: Question[], duration: number, attemptId: string) => void;
...
export const useExamStore = create<ExamState>((set, get) => ({
  ...
  initExam: (examId, questions, duration, attemptId) => {
  ...
  decrementTimer: () => {
    set((state) => {
      ...
      if (newTime <= 0) {
        return { timeRemaining: 0, isSubmitted: true };
      }
  ...
  getUnansweredCount: () => {
    const { questions, answers } = get();
    return questions.length - Array.from(answers.values()).filter(
      a => a.selectedOptions.length > 0
    ).length;
  },
